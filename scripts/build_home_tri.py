...
# Injected at build time from content/ticker.md
TICKER_ITEMS = []

# Injected at build time from content/this_day.md (today's IST date)
THIS_DAY_ITEMS = []
...
def page(lang, days, kind="home"):
    ...
    # This Day in Indian History sidebar labels (per language)
    this_day_heading = {
        "hi": "आज का इतिहास",
        "en": "This day in India",
        "ur": "آج کا تاریخی دن",
    }
    this_day_subhead = {
        "hi": "भारत के इतिहास में आज",
        "en": "TODAY IN INDIAN HISTORY",
        "ur": "بھارت کی تاریخ میں آج کا دن",
    }
    ...
    # Render the "This Day in Indian History" sidebar only on the home page,
    # only when there are entries for today. CSS hides it on narrow screens.
    if kind == "home" and THIS_DAY_ITEMS:
        body.append(f'<aside class="thisday {klass}">')
        body.append(f'<h3>{this_day_heading[lang]}</h3>')
        body.append(f'<div class="kicker">{this_day_subhead[lang]}</div>')
        for item in THIS_DAY_ITEMS:
            body.append('<div class="entry">')
            body.append(f'<span class="year">{e(item.get("year", ""))}</span>')
            body.append(f'<div class="event">{e(item.get(lang, item.get("en", "")))}</div>')
            body.append('</div>')
        body.append('</aside>')
...
home_doc = build_doc(
    recent_days,
    "home",
    "Bharat Samwad · भारत संवाद · بھارت سنواد",
)
...
archive_doc = build_doc(
    older_days,
    "archive",
    "Archive · पुरालेख · آرکائیو — Bharat Samwad",
)
