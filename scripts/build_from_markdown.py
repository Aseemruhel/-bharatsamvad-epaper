...
# ---------- regenerate homepage AND archive ----------
def regenerate_index(articles):
    ...
    # build_home_tri.py now writes BOTH index.html (recent days) and archive.html (older days)
    # into its CWD (scripts/). We move both files to the repo root afterwards.
    os.chdir(SCRIPTS)
    import runpy
    ...
    # Move per-date pages too (e.g. bharatsamvad-2026-06-01.html — no page suffix)
    date_pages = 0
    for fp in SCRIPTS.glob("bharatsamvad-????-??-??.html"):
        # exclude per-article pages like bharatsamvad-2026-06-01-page2.html
        if re.match(r"^bharatsamvad-\d{4}-\d{2}-\d{2}\.html$", fp.name):
            (ROOT / fp.name).write_text(fp.read_text(encoding="utf-8"), encoding="utf-8")
            fp.unlink(missing_ok=True)
            date_pages += 1

    tmp.unlink(missing_ok=True)

    print(f"  ✓ regenerated {' + '.join(moved)} + {date_pages} per-date page(s) with {len(articles)} articles across {len(sorted_dates)} day(s)")
