...
# Public URL of the site — used for absolute og:url + og:image.
# If the domain is not fully resolving yet, temporarily switch to:
#     SITE_URL = "https://bharatsamvad-epaper.pages.dev"
SITE_URL = "https://bharatsamwad.org"
DEFAULT_OG_IMAGE = "/og-default.jpg"   # served from the repo root
...
/* clickable masthead — keeps the look, gains a hover hint */
.home-logo-link{text-decoration:none;color:inherit;display:inline-block;transition:opacity .15s;}
.home-logo-link:hover{opacity:.65;}

/* back to home button at the bottom of every article */
.back-home-wrap{display:flex;justify-content:center;margin:28px 0 18px;}
.back-home-btn{
    ...
    border-radius:3px;
    box-shadow:3px 3px 0 rgba(38,34,28,.18);
    transition:all .15s ease;
}
.back-home-btn:hover{
    background:#731010;
    transform:translate(-1px,-1px);
    box-shadow:4px 4px 0 rgba(38,34,28,.22);
}
.lang-ur .back-home-btn{font-family:"Noto Nastaliq Urdu",serif;font-size:16px;line-height:2;}

/* Print Styles for PDF */
@media print {
    .langbar, .pdf-download-btn, .back-home-wrap, nav, .top-bar, .sidebar, .footer { display:none !important; }
...
function downloadAsPDF() {
    ...
    // Derive the article's own filename slug from the URL (e.g., "2026-07-09-page1")
    // Falls back to a generic prefix if this file is being previewed off-server.
    let pathSlug = (window.location.pathname.split("/").pop() || "")
                   .replace(/\.html?$/i, "")
                   .replace(/^bharatsamvad-/, "");
    if (!pathSlug) { pathSlug = "bharat-samwad"; }
    
    const filename = pathSlug + "-" + cleanTitle + ".pdf";
...
def render_lang(lang, A, photo_b64=None):
    ...
    <div class="masthead" style="border-bottom:2px solid var(--rule);padding:4px 0 6px;">
      <a href="index.html" class="home-logo-link" title="Home"><h1 style="font-size:36px;">{MAST[lang]}</h1></a>
    </div>
    <div class="strip">
...
{figure}{body}
    ...
    <div class="back-home-wrap">
      <a class="back-home-btn" href="index.html">{BACK_LABEL[lang]}</a>
    </div>
    <div class="foot">
...
def _attr_escape(value):
    """Escape a string for use inside an HTML attribute (including quotes)."""
    return _html_mod.escape(str(value or ""), quote=True)

def _social_meta(outfile, langs):
    """Build OG + Twitter Card meta tags from the article's English block.
    Falls back to Hindi or first available if no English block."""
    src_lang = langs.get("en") or langs.get("hi") or next(iter(langs.values()))
    og_title = src_lang.get("head") or "Bharat Samwad"
    og_desc  = src_lang.get("sub")  or "Bharat Samwad — Trilingual e-paper"
    # outfile may be a path; we only need the basename for the URL
    import os as _os
    filename = _os.path.basename(outfile)
    page_url = f"{SITE_URL.rstrip('/')}/{filename}"
    img_url  = f"{SITE_URL.rstrip('/')}{DEFAULT_OG_IMAGE}"

    return f'''<meta property="og:type" content="article">
<meta property="og:site_name" content="Bharat Samwad">
<meta property="og:title" content="{_attr_escape(og_title)}">
<meta property="og:description" content="{_attr_escape(og_desc)}">
<meta property="og:url" content="{_attr_escape(page_url)}">
<meta property="og:image" content="{_attr_escape(img_url)}">
<meta property="og:image:width" content="1200">
<meta property="og:image:height" content="630">
<meta property="og:image:alt" content="Bharat Samwad — सत्य · सृजन · सरोकार">
<meta property="og:locale" content="en_IN">
<meta name="twitter:card" content="summary_large_image">
<meta name="twitter:title" content="{_attr_escape(og_title)}">
<meta name="twitter:description" content="{_attr_escape(og_desc)}">
<meta name="twitter:image" content="{_attr_escape(img_url)}">
<link rel="canonical" href="{_attr_escape(page_url)}">'''


def build(outfile, title, langs, photo_b64=None):
    # English first in the HTML source: SEO, view-source, and screen-reader friendly
    blocks = "\n".join(render_lang(l, langs[l], photo_b64) for l in ("en", "hi", "ur"))
    social_meta = _social_meta(outfile, langs)
